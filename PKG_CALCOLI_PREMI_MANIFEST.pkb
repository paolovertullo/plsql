...
as
    ...
    -- TODO: Tutte le simulazioni sono vuote attualmente perch� le firma dei calcola sono tutte cambiate.
    ...
    begin
        ...
        --         se ci sono prendo piazzamento e il dato premio_masaf se � valorizzato
        --        se non � valorizzato allora due strade o chiamo il popola premio_masaf oppure lo calcolo con handler
    ...
    begin
        -- Questo metodo � richiamato come API dal BackEnd
    ...
    is
        ...
        l_eta_cavalli             number;                -- Per l'et� numerica
            ...
            l_eta_cavalli :=
                case
                    when instr (upper (v_dati_gara.desc_nome_gara_esterna),
                                '1 ANNO') >
                         0                   -- Aggiunto prima per specificit�
                    then
                        1
                    when instr (upper (v_dati_gara.desc_nome_gara_esterna),
                                '2 ANNI') >
...
--end if;
        ...
        case v_disciplina_id
            ...
            then
                ...
                -- Queste manifestazioni sono le finali per le rispettive et� e sono premiate MASAF.
                -- Assumiamo che le gare all'interno di queste manifestazioni specifiche per quell'et� siano quelle del campionato.
                ...
                then
                    -- Il montepremi MASAF � per la *Finale* del Trofeo (pag. 12).
                    -- Non sono specificate categorie di et� o livello per il premio della finale del Trofeo,
                    -- si presume che la gara sia la "finale" stessa.
                    -- Se ci sono pi� gare in una manifestazione "TROFEO DEL CAVALLO ITALIANO",
                    -- dovremmo identificare quale � effettivamente la "FINALE" del Trofeo.
                    ...
                    then       -- Aggiunto per essere pi� specifici sulla gara
                        l_premiata := 1;
                    end if;
                end if;
                ...
                then
                    ...
                    -- e "ogni categoria avr� un Montepremi".
                    if l_eta_cavalli in (4, 5) or v_nome_gara like 'MASAF%'
                    then
                        -- Non c'� distinzione sport/�lite per il premio di tappa nel disciplinare a pag. 7.
                        -- Quindi, se l'et� � 4 o 5, � una tappa premiata.
                        l_premiata := 1;
                    end if;
                end if;
                ...
                then
                    l_premiata := 1; --sempre premiate perch� inserisco nel file csv solo quelle a premio
                end if;
...
--                    end if;
                ...
                -- REGOLA 6: ESCLUDI LE GARE CHE HANNO FORMULA 'AGGI'  perch� nel file access sono le gare aggiunte non da premiare
                if  v_formula = 'AGGI'
                then
                        l_premiata := 0;
                end if;
...
--                end if;
                ...
                begin
                    ...
                    if v_nome_manifest_norm like '%MASAF%DRESSAGE%' and v_giornate_edizione-v_giornata_gara = 0 then
                        ...
                        --5 ANNI FINAL MASAF 2� GIORNATA che in realt� � in giornata 2 di tre quindi sarebbe prima giornata
            ...
            then
                ...
                declare
                    ...
                    v_nome_gara_normalizzato       varchar2 (255)
                                                       := v_nome_gara; -- v_nome_gara � gi� UPPER nella procedura principale
                begin
                    -- Controlla se la manifestazione � una Tappa o la Finale del Circuito Monta da Lavoro
                    if    v_nome_manifest_normalizzato =
                          'CIRCUITO MONTA DA LAVORO'
                       or v_nome_manifest_normalizzato =
                          'FINALE CIRCUITO MONTA DA LAVORO'
                    then
                        -- Ora controlla se la categoria della gara � una di quelle premiate MASAF
        ...
        then
            l_premiata := 0; --Non premiate perch� sono le manifestazioni FISE
       ...
       -- then
        ...
        case v_disciplina_id 
            ...
            then                                     -- MONTA DA LAVORO (ID 7)
                l_tipo_incentivo_gara := 0; -- Il programma MASAF � il suo circuito. Nessun incentivo AGGIUNTO a gare FITETREC generiche.
            when 2
            then                                           -- ENDURANCE (ID 2)
                l_tipo_incentivo_gara := 0; -- Nessun incentivo specifico menzionato per gare FISE generiche
            when 1
            then                              -- CIRCUITO ALLEVATORIALE (ID 1)
                l_tipo_incentivo_gara := 0;                  -- Gare gi� MASAF
            else
...
--           end if;
        ...
        -- 4. Se la GARA � di un tipo idoneo a incentivi, conta i cavalli MASAF partecipanti
            ...
            begin
                select count (*)
                 ...
                 where     cl.fk_sequ_id_dati_gara_esterna =
                       ...
                       and (vincite_fise > 0 or vincite_masaf > 0 ); -- Cavallo � MASAF e ci sono vincite FISE
            exception
                when no_data_found
                then
                    l_num_cav_masaf_incentivabili := 0;
...
begin
   ...
   -- end if;
    ...
    while idx is not null loop
        if ids(idx) = id then 
          --  if c_debug then
          --      dbms_output.put_line('>> Cavallo ' || id || ' gi� presente nella fascia, non riaggiunto.');
          --  end if;
            return;
        end if;
        idx := ids.next(idx);
...
if p_priorita_fasce_alte  then
    ...
    if p_premiabili < 3 then
        ...
        -- Questa � la logica originale "Allevatoriale", che per p < 3 
...
-- Recupero riga la formula dell'edizione
        ...
        then
            ...
            then
                ...
                if v_is_criterium then
                    ...
                    --nel solo caso di terza prova � 10000
    ...
    begin
     if c_debug
        ...
        then
            dbms_output.put_line (
                   '[FN_CALCOLA_MONTEPREMI_COMPLETO] Et� cavallo: '
                || v_eta_cavallo);
            dbms_output.put_line (
        ...
        case v_tipo_evento
            ...
            then
                ...
                then
                    v_montepremi_tot := 6 * 400 / 2; --met� va alla gara e met� alla progressione tecnica
                end if;
            when 'FINALE'
            then
                case v_eta_cavallo
...
begin
    -- Soglia dressage in base all'et�
    ...
    if v_disciplina = 4 then --SALTO AD OSTACOLI
        ...
        begin
            ...
            if v_piazzamento_limite is not null then
                ...
                -- Conta quanti con quel piazzamento sono gi� nei premiabili
                select count(*)
...
--            end loop;
--            
--            -- Se pi� di uno a parimerito, estendi
...
--    end;
    ...
    begin
        --ATTENZIONE QUI NON CI POSSONO ESSERE DML perch� � chiamata da una funzione che restituisce una table function
        ...
        then
            ...
            then
                v_dati_gara.fk_codi_categoria :=
                    case
                        ...
                        then
                            75
                        -- Alto livello: nel nome c'� "ALTO" oppure altezza elevata (>=125)
                        when instr (
                                 upper (v_dati_gara.desc_nome_gara_esterna),
                                 'ALTO') >
                             0
                        ...
                        then
                            74
                        -- Elite: nel nome "�LITE" o "GIUDIZIO", oppure codifica specifica
                        when instr (
                                 upper (v_dati_gara.desc_nome_gara_esterna),
                                 '�LITE') >
                             0
            ...
            -- Correzione per gare ELITE ma "a tempo" (in realt� sport)
...
--            end if;
        ...
        then
            ...
            -- 2. DEDUZIONE FK_CODI_CATEGORIA (Et� del cavallo)
            if v_dati_gara.fk_codi_categoria is null
            then
                v_dati_gara.fk_codi_categoria :=
                    case
            ...
            -- 2.b DEDUZIONE fk_codi_eta (Et� cavallo, usata nel calcolo premi)
            if v_dati_gara.fk_codi_eta is null
            then
                v_dati_gara.fk_codi_eta :=
                    case
        ...
        if v_dati_gara.fk_codi_tipo_evento is null or v_dati_gara.fk_codi_tipo_evento=''
            then    
                ...
                -- Se � un campionato di 6 o 7 anni
...
BEGIN
    ...
    -- MODIFICA: Inizio primo periodo = VENERD� prima della terza domenica (non la domenica stessa)
    -- Cos� includiamo le manifestazioni che iniziano venerd�/sabato
    v_inizio_primo := v_terza_domenica - 2;  -- Venerd� prima della terza domenica
...
begin
    ...
    -- Cerca nella mappa premi e assegna direttamente (premio gi� diviso per parimerito)
    for i in 1 .. p_mappa_premi.count loop
 ...
 -- Recupero riga la formula dell'edizione
    ...
    case
        ...
        v_desc_calcolo_premio := v_desc_calcolo_premio||' - Distribuzione: '||v_tipo_distrib|| 
                                 ...
                                 END ||' - Et�: '||v_eta||' anni'||u'\000A'||' - Categoria:'|| v_categoria||u'\000A';
            ...
            -- forzo la distribuzione a 10 premiati per il CSIO ROMA - MASTER TALENT  e per la FINALE CIRCUITO CLASSICO come da disciplinare
             if v_nome_manifestazione like '%CSIO%ROMA%MASTER%' or v_nome_manifestazione like '%FINALE%CIRCUITO%CLASSICO%' then
                ...
                for i in 1 .. v_classifica.count loop
                    ...
                    v_mappa_premi(i).id_cavallo := v_classifica(i).id_cavallo;
                    -- Se � un ritirato 910 non lo premio o anche un eliminato
...
begin
    ...
    if c_debug then
        dbms_output.put_line('>> 4. Nome manifestazione ed et�');
    end if;
    
    -- 4. Nome manifestazione ed et�
    select upper(mf.desc_denom_manifestazione)
      into v_nome_manifestazione
      from tc_dati_gara_esterna dg
      join tc_dati_edizione_esterna ee on ee.sequ_id_dati_edizione_esterna = dg.fk_sequ_id_dati_ediz_esterna
        ...
        if v_eta_cavalli in (5, 6, 7, 8) then
            if instr(upper(v_dati_gara.desc_nome_gara_esterna), 'PRELIMINARY') > 0 
               or instr(upper(v_dati_gara.desc_nome_gara_esterna), 'PRELIMINARE') > 0 then
                if c_debug then
                    dbms_output.put_line('>> Gara PRELIMINARY: non premiata (solo FINALE � premiata)');
                end if;
                return l_risultati;
            end if;
        end if;
    ...
    if c_debug then
        ...
        dbms_output.put_line('>> Gara: ' || v_dati_gara.desc_nome_gara_esterna);        
        dbms_output.put_line('>> Et�: ' || v_eta_cavalli || ' anni');
    ...
    if v_formula = 'FISE' then
        ...
        -- Determina percentuali e max premiati in base all'et�
...
--        end if;
--    
--        -- NUOVO CONTROLLO: Escludi gare PRELIMINARY (solo FINALE � premiata)
--        -- Per cavalli 4 anni non c'� distinzione PRELIMINARY/FINALE
--        if v_eta_cavalli in (5, 6, 7, 8) then
--            if instr(upper(v_dati_gara.desc_nome_gara_esterna), 'PRELIMINARY') > 0 
--               or instr(upper(v_dati_gara.desc_nome_gara_esterna), 'PRELIMINARE') > 0 then
--                if c_debug then
...
function handler_endurance (p_gara_id in number)
    ...
    begin
        ...
        then
            ...
            then
                if c_debug
                then
                    dbms_output.put_line ('Gara con pi� di 4 partecipanti ');
                end if;
   ...
   v_tipo_evento:= 'TROFEO';
        ...
        then
            ...
            dbms_output.put_line ('Et�: ' || v_dati_gara.fk_codi_eta);
            dbms_output.put_line (
...
begin
    ...
    begin
        ...
        -- Verifica se la gara ha gi� premi calcolati
        ...
        then
            ...
            then
                p_risultato :=
                    'Attenzione la Gara ha gi� i premi elaborati.';
            end if;

            if p_forza_elabora = 1
            then
                p_risultato :=
                    'Attenzione la Gara � definita come senza premi Masaf.';
    ...
    begin
        ...
        loop
            ...
            then
                dbms_output.put_line ('  -> Gi� elaborata, salto.');
                continue;
            end if;
    ...
    is
        ...
        v_eta_cavallo_num     number;         -- Et� numerica (1, 2, o 3 anni)
        v_id_cavallo          number;
        v_fascia              number;
    begin
        if c_debug
        ...
        v_tipo_evento_descr :=
            upper (fn_desc_tipologica (p_dati_gara.fk_codi_tipo_evento));
        v_eta_cavallo_id := p_dati_gara.fk_codi_eta; -- Assumiamo sia gi� popolato o dedotto correttamente



        -- Mappatura ID et� a numero (semplificata, da adattare se gli ID sono diversi)
...
--            end loop;
        ...
        begin
            ...
            loop
                ...
                then
                    -- Ho gi� premiato abbastanza cavalli, esco
                    exit;
                elsif premiabili.count + gruppo_temp.count > v_metaparte
                then
                    -- Sto per superare la soglia: premio solo se � un gruppo di parimerito
                    ...
                    then
                        ...
                        dbms_output.put_line (
                            '- NON aggiungo cavallo fuori soglia, non � parimerito.');
                    end if;
...
BEGIN
    ...
    IF c_debug THEN
        ...
        IF p_modalita_test THEN
             DBMS_OUTPUT.PUT_LINE ('   ** ESEGUITO IN MODALIT� TEST - NESSUN CAMBIO AI DATI DI GARA **');
        END IF;
        DBMS_OUTPUT.PUT_LINE (RPAD('=', 80, '=') || CHR(10));
    END IF;
...
IS
    ...
    -- Variabili per distribuzione premi
    v_percentuali           sys.odcinumberlist;
    v_max_premiati          PLS_INTEGER;
    v_pos_corrente          PLS_INTEGER;
    v_num_parimerito        PLS_INTEGER;
...
BEGIN
    IF c_debug THEN
        DBMS_OUTPUT.PUT_LINE(CHR(10) || RPAD('=', 80, '='));
        DBMS_OUTPUT.PUT_LINE('   HANDLER DRESSAGE V2 (TEST MODE: ' || CASE WHEN p_modalita_test THEN 'ON' ELSE 'OFF' END || ')');
        DBMS_OUTPUT.PUT_LINE('   Gara ID: ' || p_gara_id);
        DBMS_OUTPUT.PUT_LINE(RPAD('=', 80, '='));
    END IF;
    ...
    -- 4. Nome manifestazione e formula
    SELECT UPPER(mf.desc_denom_manifestazione), UPPER(mf.desc_formula)
      INTO v_nome_manifestazione, v_formula
      FROM tc_dati_gara_esterna dg
      JOIN tc_dati_edizione_esterna ee ON ee.sequ_id_dati_edizione_esterna = dg.fk_sequ_id_dati_ediz_esterna
      JOIN tc_edizione ed ON ed.sequ_id_edizione = ee.fk_sequ_id_edizione
      JOIN tc_manifestazione mf ON mf.sequ_id_manifestazione = ed.fk_sequ_id_manifestazione
    ...
    v_desc_calcolo_premio := v_desc_calcolo_premio||' - Età: '||v_eta_cavalli||' anni'||u'\000A';
    v_desc_calcolo_premio := v_desc_calcolo_premio||' - Manifestazione: '||v_nome_manifestazione||u'\000A';

    -- 5. Determina soglia e montepremi usando COSTANTI
    IF v_nome_manifestazione = C_PAT_CIRCUITO_DRESSAGE THEN
        IF v_giornata != 2 THEN
            v_desc_calcolo_premio := v_desc_calcolo_premio||' - Circuito MASAF: premiata solo giornata 2'||u'\000A';
            RETURN l_risultati;
        END IF;

        IF v_eta_cavalli IN (5, 6, 7, 8) THEN
            IF UPPER(v_dati_gara.desc_nome_gara_esterna) LIKE C_PAT_PRELIMINARY THEN
                v_desc_calcolo_premio := v_desc_calcolo_premio||' - Gara PRELIMINARY: non premiata'||u'\000A';
                RETURN l_risultati;
            END IF;
        END IF;

        v_soglia_punteggio := CASE p_anno WHEN 2025 THEN C_2025_DR_CIRC_SOGLIA ELSE C_2025_DR_CIRC_SOGLIA END;
        v_montepremi_tot := CASE v_eta_cavalli
            WHEN 4 THEN CASE p_anno WHEN 2025 THEN C_2025_DR_CIRC_4_ANNI ELSE C_2025_DR_CIRC_4_ANNI END
            WHEN 5 THEN CASE p_anno WHEN 2025 THEN C_2025_DR_CIRC_5_ANNI ELSE C_2025_DR_CIRC_5_ANNI END
            WHEN 6 THEN CASE p_anno WHEN 2025 THEN C_2025_DR_CIRC_6_ANNI ELSE C_2025_DR_CIRC_6_ANNI END
            WHEN 7 THEN CASE p_anno WHEN 2025 THEN C_2025_DR_CIRC_7_ANNI ELSE C_2025_DR_CIRC_7_ANNI END
            WHEN 8 THEN CASE p_anno WHEN 2025 THEN C_2025_DR_CIRC_8_ANNI ELSE C_2025_DR_CIRC_8_ANNI END
            ELSE 0
        END;
        v_desc_calcolo_premio := v_desc_calcolo_premio||' - Tipo: Circuito MASAF'||u'\000A';

    ELSIF v_nome_manifestazione LIKE C_PAT_FINALE_DRESSAGE THEN
        v_soglia_punteggio := CASE p_anno WHEN 2025 THEN C_2025_DR_FIN_SOGLIA ELSE C_2025_DR_FIN_SOGLIA END;
        v_montepremi_tot := CASE v_eta_cavalli
            WHEN 4 THEN CASE p_anno WHEN 2025 THEN C_2025_DR_FIN_4_ANNI ELSE C_2025_DR_FIN_4_ANNI END
            WHEN 5 THEN CASE p_anno WHEN 2025 THEN C_2025_DR_FIN_5_ANNI ELSE C_2025_DR_FIN_5_ANNI END
            WHEN 6 THEN CASE p_anno WHEN 2025 THEN C_2025_DR_FIN_6_ANNI ELSE C_2025_DR_FIN_6_ANNI END
            WHEN 7 THEN CASE p_anno WHEN 2025 THEN C_2025_DR_FIN_7_ANNI ELSE C_2025_DR_FIN_7_ANNI END
            WHEN 8 THEN CASE p_anno WHEN 2025 THEN C_2025_DR_FIN_8_ANNI ELSE C_2025_DR_FIN_8_ANNI END
            ELSE 0
        END;
        v_desc_calcolo_premio := v_desc_calcolo_premio||' - Tipo: Finale Dressage'||u'\000A';

    ELSIF pkg_calcoli_premi_manifest.fn_incentivo_masaf_gara_fise(p_gara_id) = 0 THEN
        v_desc_calcolo_premio := v_desc_calcolo_premio||' - Manifestazione non premiata'||u'\000A';
        RETURN l_risultati;
    ELSE
        v_soglia_punteggio := 0;
    END IF;

    v_desc_calcolo_premio := v_desc_calcolo_premio||' - Soglia punteggio: '||v_soglia_punteggio||'%'||u'\000A';
    v_desc_calcolo_premio := v_desc_calcolo_premio||' - Montepremi totale: '||v_montepremi_tot||' Euro'||u'\000A';

    -- 6. OTTIMIZZAZIONE: Precalcolo parimerito (evita loop in loop)
    FOR rec IN (SELECT posizione_rank, COUNT(*) AS conta
                  FROM (SELECT RANK() OVER (ORDER BY NVL(t.nume_punti, 0) DESC, t.nume_piazzamento ASC) AS posizione_rank
                          FROM tc_dati_classifica_esterna t
                         WHERE fk_sequ_id_dati_gara_esterna = p_gara_id
                           AND t.fk_sequ_id_cavallo IS NOT NULL
                           AND t.nume_piazzamento < 900)
    ...
    -- 7. Costruzione classifica con soglia punteggio
    FOR rec IN c_classifica LOOP
        IF NVL(rec.nume_punti, 0) >= v_soglia_punteggio THEN
            idx := idx + 1;
            v_classifica(idx).id_cavallo := rec.fk_sequ_id_cavallo;
    ...
    v_desc_calcolo_premio := v_desc_calcolo_premio||' - Cavalli con punteggio >= '||v_soglia_punteggio||'%: '||v_num_partenti_valido||u'\000A';

    IF v_num_partenti_valido = 0 THEN
        RETURN l_risultati;
    END IF;

    -- 8. Costruzione mappa premi
    v_idx_mappa := 0;

    IF v_formula = C_FORMULA_FISE THEN
        -- INCENTIVO 10% FISE
        FOR i IN 1 .. v_classifica.COUNT LOOP
            v_idx_mappa := v_idx_mappa + 1;
            v_mappa_premi(v_idx_mappa).id_cavallo := v_classifica(i).id_cavallo;
            v_mappa_premi(v_idx_mappa).premio := ROUND(v_classifica(i).vincite_fise *
                CASE p_anno WHEN 2025 THEN C_2025_DR_INCENTIVO_FISE_PERC ELSE C_2025_DR_INCENTIVO_FISE_PERC END, 2);
            v_mappa_premi(v_idx_mappa).fascia := v_classifica(i).posizione;
        END LOOP;
        v_desc_calcolo_premio := v_desc_calcolo_premio||' - Incentivo FISE 10%'||u'\000A';

    ELSE
        -- Determina percentuali in base all'età (USA COSTANTI)
        IF v_eta_cavalli IN (7, 8) THEN
            v_percentuali := sys.odcinumberlist(
                CASE p_anno WHEN 2025 THEN C_2025_DR_PERC_78_P1 ELSE C_2025_DR_PERC_78_P1 END,
                CASE p_anno WHEN 2025 THEN C_2025_DR_PERC_78_P2 ELSE C_2025_DR_PERC_78_P2 END,
                CASE p_anno WHEN 2025 THEN C_2025_DR_PERC_78_P3 ELSE C_2025_DR_PERC_78_P3 END
            );
            v_max_premiati := 3;
            v_desc_calcolo_premio := v_desc_calcolo_premio||' - Distribuzione: Top 3 (50%, 30%, 20%)'||u'\000A';
        ELSE
            v_percentuali := sys.odcinumberlist(
                CASE p_anno WHEN 2025 THEN C_2025_DR_PERC_ALTRI_P1 ELSE C_2025_DR_PERC_ALTRI_P1 END,
                CASE p_anno WHEN 2025 THEN C_2025_DR_PERC_ALTRI_P2 ELSE C_2025_DR_PERC_ALTRI_P2 END,
                CASE p_anno WHEN 2025 THEN C_2025_DR_PERC_ALTRI_P3 ELSE C_2025_DR_PERC_ALTRI_P3 END,
                CASE p_anno WHEN 2025 THEN C_2025_DR_PERC_ALTRI_P4 ELSE C_2025_DR_PERC_ALTRI_P4 END,
                CASE p_anno WHEN 2025 THEN C_2025_DR_PERC_ALTRI_P5 ELSE C_2025_DR_PERC_ALTRI_P5 END
            );
            v_max_premiati := 5;
            v_desc_calcolo_premio := v_desc_calcolo_premio||' - Distribuzione: Top 5 (35%, 22%, 17%, 14%, 12%)'||u'\000A';
        END IF;

        -- Costruzione mappa con parimerito OTTIMIZZATO (usa mappa precalcolata)
        v_pos_corrente := 1;
        WHILE v_pos_corrente <= v_max_premiati LOOP
            v_num_parimerito := NVL(v_parimerito(v_pos_corrente), 0);
            EXIT WHEN v_num_parimerito = 0;

            -- Calcola premio diviso
            IF v_pos_corrente <= v_percentuali.COUNT THEN
                v_premio_diviso := ROUND(v_montepremi_tot * v_percentuali(v_pos_corrente) / v_num_parimerito, 2);
            ELSE
                v_premio_diviso := 0;
    ...
    IF c_debug THEN
        DBMS_OUTPUT.PUT_LINE(CHR(10) || RPAD('=', 80, '='));
        DBMS_OUTPUT.PUT_LINE('   FINE HANDLER DRESSAGE V2 - Gara ID: ' || p_gara_id);
        IF p_modalita_test THEN
...
EXCEPTION
    WHEN OTHERS THEN
        DBMS_OUTPUT.PUT_LINE('ERRORE HANDLER_DRESSAGE_V2: ' || SQLERRM);
        RAISE;
